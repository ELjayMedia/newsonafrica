...
import { kvCache, createCacheEntry } from "@/lib/cache/kv"
...
const persistArticleToCache = async (
  ...
  try {
    await kvCache.set(key, createCacheEntry(payload), ARTICLE_CACHE_TTL_SECONDS)
  } catch (error) {
...
const readArticleFromCache = async (
  ...
  try {
    const entry = await kvCache.get<LoadedArticle>(key)
