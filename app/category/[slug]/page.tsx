...
export async function generateStaticParams() {
  ...
  try {
    const categories = await circuitBreaker.execute(
      ...
      async () => {
        logger.debug("[v0] Categories static generation: Using fallback due to WordPress unavailability")
        return []
      },
    )
...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  if (cached.exists && !cached.isStale) {
    logger.debug(`[v0] Category metadata: Using cached data for ${params.slug}`)
    return cached.data
  }
  ...
  try {
    const result = await circuitBreaker.execute(
      ...
      async () => {
        if (cached.exists) {
          logger.debug(`[v0] Category metadata: Using stale cache for ${params.slug}`)
          return cached.data
        }

        logger.debug(`[v0] Category metadata: Using fallback for ${params.slug}`)
        return {
...
export default async function CategoryServerPage({ params }: CategoryPageProps) {
  ...
  try {
    const categoryData = await circuitBreaker.execute(
      ...
      async () => {
        if (cached.exists) {
          logger.debug(`[v0] Category data: Using stale cache for ${params.slug}`)
          return cached.data
        }

        logger.debug(`[v0] Category data: Using fallback for ${params.slug}`)
        return {
          category: {
    ...
    if (cached.exists) {
      logger.debug(`[v0] Category page: Using cached data due to error for ${params.slug}`)
