...
export async function GET(request: NextRequest) {
  ...
  try {
    logger.debug("Making request to Paystack API")
    ...
    logger.debug("Paystack API response status:", response.status)
    ...
    if (data.status && data.data.status === "success") {
      try {
        ...
        logger.debug("Subscription verified and stored:", data.data.reference)
      } catch (dbError) {
