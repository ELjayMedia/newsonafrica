...
export async function GET(request: NextRequest) {
  ...
  try {
    logger.info("Making request to Paystack API")
    ...
    logger.info("Paystack API response status:", response.status)
    ...
    if (data.status && data.data.status === "success") {
      try {
        ...
        logger.info("Subscription verified and stored:", data.data.reference)
      } catch (dbError) {
