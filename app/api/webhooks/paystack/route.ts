...
export async function POST(request: Request) {
  try {
    ...
    logger.debug(`Received Paystack webhook: ${event.event}`)
    ...
    switch (event.event) {
      ...
      default:
        logger.debug(`Unhandled Paystack event: ${event.event}`, event.data)
    }
...
export async function handleChargeSuccess(
  ...
  logger.debug("Processing successful charge:", data.reference)
...
export async function handleSubscriptionCreated(
  ...
  logger.debug("Processing subscription creation:", data.subscription_code)
  const userId = await getUserIdByEmail(client, data.customer.email)
...
export async function handleSubscriptionDisabled(
  ...
  logger.debug("Processing subscription cancellation:", data.subscription_code)
...
export async function handlePaymentFailed(
  ...
  logger.debug("Processing failed payment:", data.reference)
  const userId = await getUserIdByEmail(client, data.customer.email)
...
export async function handleInvoiceUpdate(
  ...
  logger.debug("Processing invoice update:", data.invoice_code)
  const userId = await getUserIdByEmail(client, data.customer.email)
  const { error } = await client
    .from("subscriptions")
...
export async function handleTransferSuccess(
  ...
  logger.debug("Processing successful transfer:", data.reference)
...
export async function handleTransferFailed(
  ...
  logger.debug("Processing failed transfer:", data.reference)
  const { error } = await (client as any)
    .from("transfers")
