...
export async function GET(request: Request, { params }: { params: { country: string } }) {
  ...
  const items = posts
    ...
    .map(p => {
      ...
      return `<url><loc>${url}</loc><news:news><news:publication><news:name>News On Africa – ${params.country.toUpperCase()}</news:name><news:language>en</news:language></news:publication><news:publication_date>${p.date}</news:publication_date><news:title>${title}</news:title></news:news></url>`;
    });
