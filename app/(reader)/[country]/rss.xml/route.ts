...
export async function GET(request: Request, { params }: { params: { country: string } }) {
  const posts = await WPR.list({ country: params.country, perPage: 20 });
  const items = posts
    .map(p => {
      const link = canonicalUrl(params.country, `/${p.categorySlugs[0] || ''}/${p.slug}`);
    ...
    .join('');
  const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0"><channel><title>News On Africa – ${params.country.toUpperCase()}</title><link>${canonicalUrl(params.country, '/')}</link><description>Latest news</description>${items}</channel></rss>`;
