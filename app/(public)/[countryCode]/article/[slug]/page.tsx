...
if (resolvedArticle.status !== "found") {
  // This should never happen at runtime after the guards above, but helps TS understand
  notFound(); // or throw new Error("Unexpected article state")
}

// Now TypeScript knows resolvedArticle is the "found" branch → .sourceCountry exists
const { sourceCountry, canonicalCountry, article } = resolvedArticle; // or just use resolvedArticle.sourceCountry

// ... rest unchanged

const targetCountry = isCountryEdition(edition)
  ? normalizeRouteCountry(canonicalCountry ?? sourceCountry ?? editionCountry)
  : routeCountry;
...
const relatedCountry = sourceCountry ?? editionCountry;
// ... relatedPosts fetching ...
...
return (
  <>
    ...
    <ArticleClientContent
      slug={normalizedSlug}
      countryCode={targetCountry}
      sourceCountryCode={sourceCountry}           // ← now safe
      initialData={articleData}
      relatedPosts={relatedPosts}
    />
  </>
