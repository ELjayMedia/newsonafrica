...
- `METRICS_FORWARD_URL` (optional, server-only) – destination webhook that receives observability envelopes captured by `/api/metrics`

## Workflow Overview

1. **CI Validation** – linting, type-checking, tests, and `npm run analyze` all run in `.github/workflows/test.yml`. The analyze step emits bundle reports to `.next/analyze`, which are uploaded as a `bundle-analyzer` artifact on each run.
2. **Web Deploy** – builds and deploys the application to Vercel

### Bundle Analyzer Artifacts

- Run `npm run analyze` locally to reproduce the bundle analyzer output.
- Download the `bundle-analyzer` artifact from the CI run summary to inspect `client.html`/`server.html` bundle breakdowns generated by `@next/bundle-analyzer`.
