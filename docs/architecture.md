...
- **Sitemap Fetch Limits**: `/sitemap.xml` now runs as a dynamic metadata route (revalidated every five minutes) so the sitemap is assembled on demand instead of during the build. `/server-sitemap.xml` continues to pull the latest 100 posts per request; rely on cached GraphQL feeds (e.g., the aggregated data from `fetchAggregatedHome`) or native WordPress sitemaps for deeper archives to avoid heavy GraphQL bursts during builds.
