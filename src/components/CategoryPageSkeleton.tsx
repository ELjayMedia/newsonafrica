...
export const CategoryPageSkeleton = memo(function CategoryPageSkeleton() {
  ...
  return (
    <div className="space-y-8 px-4 py-4 max-w-7xl mx-auto">
      ...
      <Skeleton
        className="h-4 w-2/3 mb-8 mx-auto"
        animation="shimmer"
        delay={getStaggeredDelay(1)}
      />
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-4">
        <Skeleton
          ...
          animation="shimmer"
          delay={getStaggeredDelay(2)}
        />
        <Skeleton
          className="h-6 w-24 rounded-full"
          animation="shimmer"
          delay={getStaggeredDelay(3)}
        />
        <Skeleton
          className="h-6 w-20 rounded-full"
          animation="shimmer"
          delay={getStaggeredDelay(4)}
        />
        <Skeleton
          className="h-6 w-28 rounded-full"
          animation="shimmer"
          delay={getStaggeredDelay(5)}
        />
      </div>
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="bg-white rounded-lg shadow-md overflow-hidden">
            <Skeleton
              ...
              animation="shimmer"
              delay={getStaggeredDelay(i + 7)}
            />
            <div className="p-4 space-y-2">
              <Skeleton
                className="h-5 w-3/4"
                animation="shimmer"
                delay={getStaggeredDelay(i + 13)}
              />
              <Skeleton
                className="h-4 w-full"
                animation="shimmer"
                delay={getStaggeredDelay(i + 19)}
              />
              <Skeleton
                className="h-4 w-1/2"
                animation="shimmer"
                delay={getStaggeredDelay(i + 25)}
              />
            </div>
          </div>
        ))}
          ...
          >
            <div className="flex-1 space-y-2 mr-3">
              <Skeleton
                className="h-4 w-3/4"
                animation="shimmer"
                delay={getStaggeredDelay(i + 33)}
              />
              <Skeleton
                className="h-3 w-1/2"
                animation="shimmer"
                delay={getStaggeredDelay(i + 37)}
              />
            </div>
            <Skeleton
              className="w-20 h-20 flex-shrink-0"
              animation="shimmer"
              delay={getStaggeredDelay(i + 41)}
            />
          </div>
        ))}
      </div>
