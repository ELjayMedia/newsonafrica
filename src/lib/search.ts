...
const searchResultsCache = new SimpleCache<SearchResponse>(
...
const FALLBACK_POSTS: SearchPost[] = [
  {
    ...
    excerpt: {
      rendered:
        "We're experiencing difficulties with our search service. Please try again in a few moments.",
    },
...
export async function searchPosts(
  ...
  try {
    ...
    if (first.res && first.res.results.length >= limit) {
      ...
      const mergedResults = [...(first.res?.results || []), ...(other?.results || [])].slice(
        0,
        limit,
      );
