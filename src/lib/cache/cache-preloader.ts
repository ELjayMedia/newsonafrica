...
class ConcurrencyLimiter {
  ...
  async run<T>(fn: () => Promise<T>): Promise<T> {
    ...
    this.active++;
    console.debug(`[CachePreloader] Active preload tasks: ${this.active}/${this.maxConcurrent}`);

    if (this.active > this.maxConcurrent) {
      console.warn(`[CachePreloader] Concurrency exceeded ${this.maxConcurrent}: ${this.active}`);
    }
...
class CachePreloader {
  ...
  async smartPreload(
    ...
    const sortedPosts = postsToPreload
      .sort((a, b) => a.priority - b.priority)
      .map(({ priority, ...post }) => post);
