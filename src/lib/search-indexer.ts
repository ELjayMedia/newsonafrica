...
export class SearchIndexer {
    ...
    try {
      ...
      for (let i = 0; i < posts.length; i += SEARCH_CONFIG.PERFORMANCE.BATCH_SIZE) {
        ...
        if (Date.now() - startTime > SEARCH_CONFIG.PERFORMANCE.MAX_INDEX_BUILD_TIME) {
          console.warn(
            `Index build taking too long, processing remaining ${posts.length - i} posts in background`,
          );
      ...
      console.log(
        `Optimized search index built in ${buildTime}ms for ${this.index.posts.size} posts`,
      );
      console.log(
  ...
  private indexWords(post: IndexedPost): void {
    ...
    const limitedContentWords = contentWords.slice(
      0,
      SEARCH_CONFIG.WORD_PROCESSING.MAX_WORDS_PER_POST,
    );
  ...
  search(query: string, options: SearchOptions = {}): IndexedPost[] {
    ...
    phrases.forEach((phrase) => {
      ...
      if (phraseWords.length >= 2) {
        ...
        bigramMatches.forEach((postId) => {
          postScores.set(
            postId,
            (postScores.get(postId) || 0) + SEARCH_CONFIG.SCORING.BIGRAM_MATCH * 2,
          );
        });
      }
    });
    ...
    searchWords.forEach((word) => {
      ...
      titleMatches.forEach((postId) => {
        postScores.set(
          postId,
          (postScores.get(postId) || 0) + SEARCH_CONFIG.SCORING.TITLE_EXACT_MATCH,
        );
      });
      ...
      contentMatches.forEach((postId) => {
        postScores.set(
          postId,
          (postScores.get(postId) || 0) + SEARCH_CONFIG.SCORING.CONTENT_EXACT_MATCH,
        );
      });
    ...
    postScores.forEach((score, postId) => {
      ...
      if (categories.length > 0) {
        const hasMatchingCategory = categories.some((catId) =>
          post.categories.includes(Number(catId)),
        );
  ...
  needsUpdate(): boolean {
    ...
    return (
      (now - this.index.lastUpdated > updateInterval || this.index.posts.size === 0) &&
      !this.isBuilding
    );
  }
  ...
  private calculateAveragePopularity(): number {
    ...
    const totalPopularity = Array.from(this.index.posts.values()).reduce(
      (sum, post) => sum + post.popularity,
      0,
    );
  ...
  async searchPosts(
    ...
    try {
      ...
      const response = await fetch(
        `${siteConfig.wordpress.apiUrl}/posts?${searchParams.toString()}`,
      );
