        ...
        if (parentComment) {
          ...
          try {
            ...
            if (post) {
              ...
              const response = await fetch(`${WORDPRESS_REST_API_URL}/posts/${comment.post_id}`);
              if (response.ok) {
...
export async function reportComment(data: ReportCommentData): Promise<void> {
  ...
  if (!hasStatus) {
    throw new Error(
      'Comment reporting requires database migration. Please run the migration script first.',
    );
  }
