      ...
      {Array.from({ length: 4 }).map((_, i) => (
        ...
        >
          <div className="flex flex-col justify-between flex-1 pr-4">
            <Skeleton className="h-4 w-3/4 mb-2" animation="shimmer" delay={getDelay(i * 2)} />
            <Skeleton className="h-3 w-1/2" animation="shimmer" delay={getDelay(i * 2 + 1)} />
          </div>
          <Skeleton
            className="w-[62px] h-[62px] flex-shrink-0 self-center"
            animation="shimmer"
            delay={getDelay(i * 2 + 2)}
          />
        </div>
      ))}
    </div>
