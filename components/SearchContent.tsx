...
export function SearchContent({ initialQuery = "" }: SearchContentProps) {
  ...
  const performSearch = useCallback(async (query: string, page = 1, append = false) => {
    ...
    setIsLoading(true)
    logger.debug(`Performing search for: "${query}", page: ${page}, append: ${append}`)

    try {
      const response = await fetch(`/api/search?q=${encodeURIComponent(query)}&page=${page}&per_page=20`)
      logger.debug(`Search API response status: ${response.status}`)
      ...
      logger.debug("Search API response data:", data)
      ...
      logger.debug(`Search completed: ${data.results?.length || 0} results found`)
    } catch (error) {
