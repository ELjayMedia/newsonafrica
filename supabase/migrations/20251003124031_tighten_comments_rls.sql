...
CREATE POLICY IF NOT EXISTS "Edition readers can read comments" ON public.comments
  FOR SELECT USING (
    country = COALESCE(current_setting('request.jwt.claims.country', true), 'african-edition')
  );
...
CREATE POLICY IF NOT EXISTS "Users can insert own comments" ON public.comments
  FOR INSERT WITH CHECK (
    auth.uid() = user_id
    AND country = COALESCE(current_setting('request.jwt.claims.country', true), 'african-edition')
  );
