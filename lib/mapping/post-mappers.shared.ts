// GraphQL post mapper utilities that are safe to import from shared/client code.
...
export type GraphqlPostMapperOptions = {
  normalizeContent?: (content: string, countryCode?: string) => string
}
...
const mapGraphqlAuthor = (post: GraphqlPostNode): WordPressAuthor | undefined => {
  ...
  const resolvedNodeId =
    typeof rawNodeId === "number" ? rawNodeId : typeof rawNodeId === "string" ? decodeGlobalId(rawNodeId) : undefined
...
export const mapGraphqlPostToWordPressPost = (
  ...
  countryCode?: string,
  options: GraphqlPostMapperOptions = {},
): WordPressPost => {
  ...
  return {
    ...
    content:
      typeof rawContent === "string" && rawContent.length > 0
        ? normalizeContent
          ? normalizeContent(rawContent, countryCode)
          : rawContent
        : undefined,
